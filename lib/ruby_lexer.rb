...
#           return '?'
      ...
      when '$' then
        ...
        when /[~*$?!@\/\\;,.=:<>\"]/ then
          token_buffer <<('$')
          token_buffer <<(c)
          self.yacc_value = t(token_buffer.to_s)
          return :tGVAR
        when '-' then
            token_buffer << '$'
          token_buffer << c
...
#         end
      ...
      peek = src.read;
      if c =~ /\!|\?/ && token_buffer[0] =~ /\w/ && peek != '=' then
        src.unread(peek);
        token_buffer <<(c);
      else
        src.unread(peek) unless peek.nil?
        src.unread c
      end
      ...
      when '@' then
        ...
        if (last == '!' || last == '?') then
          ...
          if result == 0 && token_buffer[0] =~ /[A-Z]/ then
            result = :tCONSTANT;
          else
            result = :tIDENTIFIER;
          end
