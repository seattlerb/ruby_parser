...
class RubyLexer
  ...
  def reset(reset_state=true)
    if reset_state
      # Unit tests set states prior to resetting lexer.
  ...
  def advance_and_decorate
    ...
    puts decorate(location,
                  "\e[0;32m#{type} #{val.inspect} #{@cond} #{@cmdarg}\e[0m")
  ...
  def error(message)
    raise RubyParser::SyntaxError, message
  end
